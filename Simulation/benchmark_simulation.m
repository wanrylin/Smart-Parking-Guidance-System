...
%% the developing version of benchmark system
% 1.recommendation system is designed based to always give the closest
% parking lot that hasn't try
...
%directly read the dataset same as proposed system
load("testset1.mat");
dataset = [dataset,ones(length(dataset),1)];
...
for i = 1:14
    ...
    % adjust buffer
    model_addition = ones([park_size(2),1]);
...
%     % save lotmodel into parking_lot
    parking_lot{1,i} = [parking_lot{1,i},model_addition];
end
...
for i = index_min:index_max
    ...
    if ismember(i,index)
        ...
        for n = 1:frame_length
            ...
            recommended_lot = greedy(departure_sample,walk_time);
            % save recommended_lot variable in the onroad buffer
            onroad = [onroad;[0,recommended_lot,departure_sample]];
        end
    ...
    if ismember(i,onroad(:,4))
        ...
        for n = 1:arrivals_length
            ...
            nonZeroRows = any(parking_lot{1,lot_num}(:,1:6), 2);
            ...
            if occupancy_num > full_percentage * ceil(park_capacity(lot_num,2))
                ...
                parking_sample(1,6) = parking_sample(1,6) + 1; % search_times + 1
                recommended_lot = greedy(parking_sample,walk_time);
                % resave recommended_lot variable in the onroad buffer
                parking_time = parking_time + waste_time;
                onroad = [onroad;[parking_time,recommended_lot,parking_sample]];
                ...
                park_time = exp(log(1 + park_capacity(lot_num,2)/120) * occupancy_num/park_capacity(lot_num,2));
                parking_time = parking_time + park_time + walk_time(lot_num,parking_sample(1,4));
...
save("1st_try_benchmark.mat","left","waste");
